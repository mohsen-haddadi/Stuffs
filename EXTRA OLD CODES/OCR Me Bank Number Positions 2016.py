...
My_Profile_Name = "Canary7PP"
My_Seat_Number = 3 #None
...
def OCR_My_Bank_Number(Seat_Num):
    ...
    if not( string1.count("$")==1 and string1.find('$')==0 and string2.isdigit()
        ...
        if not(x1==False and x2==False) :
            print("OCR_My_Bank_Number(This case can not happen 1 ! My Bank can't be read, Becuase i'm in winning light) Seat %s" %Seat_Num)
        ...
        if ( x1==False and x2==False ) :
            ...
            if not( string1.count("$")==1 and string1.find('$')==0 and string2.isdigit()
                    and x1==False and x2==False ) :
                if not(x1==False and x2==False) :
                    print("OCR_My_Bank_Number(This case can not happen 2 ! My Bank can't be read, Becuase i'm in winning light) Seat %s" %Seat_Num)
                    ...
                    print("OCR_My_Bank_Number(Error Not Digit!!2) Seat %s" %Seat_Num)
                    Screenshot_Error( "OCR_My_Bank_Number(Error Not Digit!!2) Seat %s" %Seat_Num  ) #Type_of_Error in string
                    return None
                else :
                    print("OCR_My_Bank_Number(Unknown Error) Seat %s" %Seat_Num )
